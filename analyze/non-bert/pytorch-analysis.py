...
TXT_REPORT_FILE = "result/semantic_similarity_report.txt"
CSV_RESULT_FILE = "result/semantic_similarity_results.csv"
...
def main():
    ...
    print("="*60)
    print(f"🖥️ 当前使用的计算设备: 【{device.upper()}】")
    if device == "cpu":
        print("⚠️ 警告: 未检测到可用的 NVIDIA 显卡 (CUDA)。")
        print("💡 如果你有 N卡，请在终端运行以下命令安装 GPU 版 PyTorch：")
        print("pip install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cu121")
    print("="*60)

    print(f"\n⏳ 正在加载高维语义模型[{EMBEDDING_MODEL_NAME}] ...")
    # 明确指定把模型加载到 GPU 还是 CPU
